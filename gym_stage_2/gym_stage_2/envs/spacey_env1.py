...
class FooEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.defined_set_of_points = [[100, 100], [300, 100], [500, 100]]#, [100, 200], [300, 200], [500, 200], [100, 300], [300, 300], [500, 300]]
        ...
        self.low = np.array([self.min_position, self.min_y_position, self.min_position, self.min_y_position, 0])#, 0, 0])
        self.high = np.array([self.max_position, self.max_y_position, self.min_position, self.min_y_position, 6.3])#, self.screen_width, self.screen_height])
    ...
    def step(self, action):
        ...
        if self.rocket_in_planet_range(position, position_y):
            ...
            if done:
                reward = -1
        self.state = (position , position_y , self.target_planet_center[0], self.target_planet_center[1],rot)
        normalized_state = ((position) / self.screen_width, (position_y)/self.screen_height, self.target_planet_center[0] / self.screen_width, self.target_planet_center[1] / self.screen_height, rot / 6.28319)
    ...
    def reset(self):
        ...
        self.state = np.array([self.origin_planet_center[0] , self.origin_planet_center[1] + self.planet_radius, self.target_planet_center[0], self.target_planet_center[1],0])
        normalized_state = np.array([(self.origin_planet_center[0])/screen_width, (self.origin_planet_center[1] + self.planet_radius ) / screen_height, self.target_planet_center[0] / screen_width, self.target_planet_center[1] / screen_height ,0])
